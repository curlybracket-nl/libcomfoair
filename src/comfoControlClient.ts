 ...
 */
const SESSION_EXEMPT_OPCODES = [
    Opcode.REGISTER_DEVICE_REQUEST,
    Opcode.START_SESSION_REQUEST,
    Opcode.KEEP_ALIVE,
];
    ...
    >(opcode: T, data?: TRequest) : Promise<ComfoControlMessage<R>> {
        ...
        return this.pendingReplies[requestId].then((response) => {
            ...
            if (response.opcode !== responseOpcode) {
                throw new Error(`Unexpected response opcode: ${Opcode[response.opcode]} (expected: ${Opcode[responseOpcode]})`);
            }
